...
describe('Paths run', () => {
  ...
  const DEFAULT_REACT_JSX_DIR = resolve(
    process.cwd(),
    'icon-library-alt/react/jsx'
  );
  const CSS_ICON_FILE_PATH = resolve(
    process.cwd(),
    'icon-library-alt/css/icon-company.css'
  );
  const JS_ICON_FILE_PATH = resolve(
    process.cwd(),
    'icon-library-alt/react/jsx/IconCompany.jsx'
  );
  ...
  before(() => {
    ...
    const child = spawnSync(
      ...
      [
        ...
        './icon-library-alt',
        '--csspath',
        './tests/css-mock-template.eta',
        '--jspath',
        './tests/react-jsx-mock-template.eta',
        '--license',
        './LICENSE',
      ],
      {
  ...
  it('Uses custom css template', async () => {
    const cssChars = await readFile(CSS_ICON_FILE_PATH, 'utf8');
...
describe('Default license file run', () => {
  ...
  const DEFAULT_LICENSE_FILE_PATH = resolve(
    process.cwd(),
    'icon-library/LICENSE.txt'
  );
